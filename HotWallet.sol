...
import "./HotWalletManager.sol";

contract HotWallet {
    event depositForwarded(address executer, uint256 value, bytes data);
    event depositDone(address from, uint256 value, bytes data);
    ...
    address hotWalletManagerAddress;
    ...
    modifier onlyAdmin() {
        address adminAddress = HotWalletManager(hotWalletManagerAddress)
        .showAdminAddress();
    ...
    modifier onlyWhenNotPause() {
        bool pauseSituation = HotWalletManager(hotWalletManagerAddress)
        .showPauseSituation();
        require(!pauseSituation, "It's been paused");
        _;
    }

    function init(
        address[] calldata allowedSigners,
        address _hotWalletManagerAddress
    ) external onlyUninitialized {
        require(allowedSigners.length == 3, "Invalid number of signers");
        hotWalletManagerAddress = _hotWalletManagerAddress;
    ...
    fallback() external payable {
        if (msg.value > 0) {
            HotWalletManager(hotWalletManagerAddress).DepositDoneEvent(
                msg.sender,
                msg.value,
                address(this),
                msg.data
            );
            emit depositDone(msg.sender, msg.value, msg.data);
        }
    }
    ...
    function withdrawTokens(
        ...
        address _to,
        uint256 _value
    ) public payable onlyAdmin onlyWhenNotPause {
        require(_value > 0);
    ...
    function withdrawBnb(address _to, uint256 _value)
    public
    payable
    onlyAdmin
    onlyWhenNotPause
    ...
    ) public payable lockCall onlyAdmin onlyWhenNotPause {
        require(_tokenContractAddress != address(0));
    ...
    function batchBnb(address[] calldata recipients, uint256[] calldata values)
    public
    payable
    lockCall
    onlyAdmin
    ...
    function recover(address _to, uint256 _value)
    public
    onlyAdmin
    returns (bytes memory)
    {
        (, bytes memory returnData) = _to.call{value : _value}("");
        return returnData;
    }
